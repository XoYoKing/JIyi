...
@synthesize tv,tableArray,label;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        label = [[labelUse alloc]initWithFrame:CGRectMake(0, 0, frame.size.width, 30)];
        
//        textField.borderStyle = UITextBorderStyleRoundedRect;//设置文本框的边框风格;
       label.userInteractionEnabled = YES;

        
        [label addTarget:self action:@selector(dropdown) forControlEvents:UIControlEventTouchUpInside];
...
//    label.text = [tableArray objectAtIndex:[indexPath row]];
    self.label.text = [tableArray objectAtIndex:[indexPath row]];
    
    NSString *str = self.label.text;
   
    
    if (self.delegate !=nil && [self.delegate respondsToSelector:@selector(getLabelText:)]) {
        
        [self.delegate getLabelText:str];
